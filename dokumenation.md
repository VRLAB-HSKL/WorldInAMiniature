...
der virtuellen Welt verfolgt werden. Dadurch ist es möglich Objekte inder virtuellen Welt zu greifen. Da ein Mensch keine unendlich langen Arme besitzt, ist die 
...
dieser virtuellen Welt. Diese wird überlicherweise auf einer kleinen Platte gerendert, welche der Benutzer in der Hand halten kann. Die Objekte auf dieser Platte 
...
Es wurde ein Teil einer kleinen Stadt erstellt, innerhalb welcher sich der Nutzer bewegen kann. Die WIM wurde auf einmal auf einem nicht beweglichen Tisch platiert und eine 
weitere WIM auf einer beweglichen Platte. Diese Platte kann vom Benutzer gegriffen werden und bewegt werden. Außerdam kann die bewegliche Platte jederzeit zum Nutzer
...
- Für das Bewegen des Nutzers wurde "post-mortem update" verwendet, da "immediate update" zu disorientierung des Benutzers führen kann [3]

## Implementierung

### Idee
Wenn die Anwendung gestartet wird, werden festgelegte Objekte geclont, in der Größe runterskaliert und auf einer kleinen Platte gerendert. Die Position und Rotation zwischen den geclonten Objekten und den realen Objekten soll synchron gehalten werden.

### Umsetzung

#### Objekte zum Klonen festlegen
Das Klonen der Objekte nach dem Start der Anwendung übernimmt das Skript `MiniWorld`. Über eine Liste von GameObjects können die Objekte festgelegt werden, welche in der Miniaturansicht inkludiert werden sollen. In der `Start`-Methode von `MonoBehaviour` werden die festgelegten Objekte geclont. Dabei wird als Parent-Objekt, das Objekt, welches das `MiniWorld` Skript besitzt angegeben.

#### Skalierung
Um die Objekte runter zu skalieren, wurde ein leeres GameObject verwendet, welches die 3D Miniatur beinhaltet. Dieses Objekt dient als Root-Objekt für alle geclonten Objekte. Bei dem Transform dieses Objekts wird dann der Scale auf den gewünschten Wert angepasst. Dadurch werden alle Kindobjekte entsprechend skaliert.

#### Positionsverfolgung von Objekten
Damit die Objekte in der 3D Miniatur an der richtigen Position stehen, entspricht die lokale Position der Objekte der lokalen Position der entsprechenden realen Objekte. Damit die Positionen der Miniaturen und realen Objekte synchron gehalten wird, wurde das Skript `TransformTracker` geschrieben. Dieses wird dem geclonten Objekt zugewiesen und dem Parameter `target` wird das entsprechende reale Objekt zugewiesen. Dadurch verfolgt das geclonte Objekt das target Objekt. Zusätzlich kann `updateTarget` gesetzt werden, um umgekehrt auch das `target` zu aktualisieren, wenn der Klon die Position ändert. Dieses Skript wird im Skript `MiniWorld` zu Beginn der Anwendung jedem geclonten Objekt hinzugefügt.
...
1. In der eigenen Scene eine neues leeres Objekt erstellen und das `MiniWorld` Skript diesem Objekt hinzufügen. Die Positionierung ist nicht relevant, da diese zur Laufzeit an die von `origin` angepasst wird
